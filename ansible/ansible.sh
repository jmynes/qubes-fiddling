...
################################################
echo -e "\n[dom0 - ansible.sh]: Create non-disposable [mgmtvm] qube, with 8GB RAM, 4vCPUs, and no networking"
...
echo -e "\n[dom0 - ansible.sh]: Set [mgmtvm] qube to 20GiB of private storage"
qvm-volume resize mgmtvm:private 20GiB

echo -e "\n[dom0 - ansible.sh]: Copying policies for qubes-ansible into /etc/qubes/policy.d and /etc/qubes/policy.d/include"
...
echo -e "\n[dom0 - ansible.sh]: Tagging created-by-mgmtvm on all default qubes and templates"
...
#echo -e "\n[dom0 - ansible.sh]: Tagging created-by-mgmtvm on additional sys-usb qubes"
#qvm-tags sys-usb-2 add created-by-mgmtvm
#qvm-tags sys-usb-3 add created-by-mgmtvm

echo -e "\n[mgmtvm - ansible.sh]: Installing qubes-ansible-vm"
qvm-run --pass-io mgmtvm "sudo dnf install qubes-ansible-vm"

echo -e "\n[dom0 - ansible.sh]: Copying playbooks to mgmtvm"
qvm-copy-to-vm mgmtvm ./playbooks 

echo -e "\n[dom0 - ansible.sh]: Copying inventory.ini to mgmtvm"
qvm-copy-to-vm mgmtvm ./inventory.ini

echo -e "\n[mgmtvm - ansible.sh]: Staging a directory for ansible files"
qvm-run --pass-io mgmtvm "mkdir -p ~/ansible/playbooks"

echo -e "\n[mgmtvm - ansible.sh]: Moving ansible files into place"
